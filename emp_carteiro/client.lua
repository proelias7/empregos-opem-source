...
job = Tunnel.getInterface("emp_carteiro")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(ped), -424.05,-2789.53,6.39,true) <= 15 then
			wait = 5
			...
			if distance2 <= 50 and pegando and encomendapega then
				if distance2 < 1.5 and not IsPedInAnyVehicle(ped) then
                    ...
                    if IsControlJustPressed(0,38) then
                    	if quantidade >= 0 then
                    		Fade(1200)
                    		local pagamento = math.random(425,630)
