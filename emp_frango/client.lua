...
emP = Tunnel.getInterface("emp_frango")
...
local destination = {
	{ x = -2173.55, y = 4282.2, z = 49.12 }, 
    { x = -584.90, y = -896.99, z = 25.94 }, 
    { x = -1037.65, y = -1397.13, z = 5.55 }, 
    { x = -1178.33, y = -891.70, z = 13.75 }, 
    { x = -1535.02, y = -454.25, z = 35.92 }, 
    { x = 90.68, y = 298.44, z = 110.22 }, 
    { x = 1249.15, y = -350.50, z = 69.20 }, 
    { x = 95.28, y = -1810.24, z = 27.09 }, 
	{ x = 176.31, y = -1460.12, z = 29.24 }
}
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Citizen.CreateThread(function()
	while true do
		...
		local distancia = GetDistanceBetweenCoords(GetEntityCoords(ped),-67.38,6268.98,31.32)	
		if distancia <= 10.0 and veiculospaw == false and onjob then
			wait = 5
			DrawMarker(21,-67.38,6268.98,31.32-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,129,242,248,150,0,0,0,1)
...
RegisterNetEvent("confirmadofrango")
AddEventHandler("confirmadofrango", function()
   SpawnVan()
end)
...
Citizen.CreateThread(function()
	while true do
		wait = 1000
		DrawMarker(21,-69.88,6255.9,31.09-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,129,242,248,150,0,0,0,1)
		if GetDistanceBetweenCoords(-69.88,6255.9,31.09, GetEntityCoords(LocalPed())) < 1.0 then
			wait = 5
			basiccheck()
		end
		DrawMarker(21,-67.8,6254.01,31.1-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,129,242,248,150,0,0,0,1)
		if GetDistanceBetweenCoords(-67.8,6254.01,31.1, GetEntityCoords(LocalPed())) < 1.0 then
			wait = 5
			checknotas()
		end
		if onjob == true then 				
			wait = 5
			DrawMarker(21,-67.43,6266.02,31.1-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,129,242,248,150,0,0,0,1)
			if GetDistanceBetweenCoords(destination[l].x,destination[l].y,destination[l].z, GetEntityCoords(GetPlayerPed(-1))) < 30.0 then
				DrawMarker(21,destination[l].x, destination[l].y, destination[l].z,0,0,0,0.0,0,0,0.5,0.5,0.4,129,242,248,150,0,0,0,1)		
			end
...
Citizen.CreateThread(function()
	...
	wait = 1000
		...
		if onjob then	
			wait = 5
			if GetDistanceBetweenCoords(myCoords.x, myCoords.y, myCoords.z, -67.43,6266.02,31.1, true) < 0.5 and onjob then
				nearBoxFarm = true
				openMenuBox(nearBoxFarm)
			elseif GetDistanceBetweenCoords(myCoords.x, myCoords.y, myCoords.z, -67.43,6266.02,31.1, true) >= 0.5 and onjob then
				nearBoxFarm = false
				openMenuBox(nearBoxFarm)
			end
		end	
...
Citizen.CreateThread(function()
	...
	wait = 1000
		...
		if model == 1945374990 and onjob and CanLeaveItem then	
			wait = 5
			coordsCar = GetOffsetFromEntityInWorldCoords(veh, 0.0, -4.00, 0.0)	
		...
		if model == 1945374990 and onjob and not CanLeaveItem then
			wait = 5
			coordsCar2 = GetOffsetFromEntityInWorldCoords(veh, 0.0, -4.00, 0.0)	
...
function basiccheck()
	if onjob == false then 
		...
		if IsControlJustPressed (1, 38) then
			...
			ExecuteCommand("roupas frango")
			SetTimeout(2000,function()
				TriggerEvent('cancelando',false)
				vRP._DeletarObjeto()
				ClearPedTasksImmediately(PlayerPedId())
				TriggerServerEvent('iniciofrango:sucesso')
...
Citizen.CreateThread(function()
	while true do
		...
		local coordsStock = GetOffsetFromEntityInWorldCoords(veh, 0.0, -4.0, 0.0)
...
function openMenuDeliveryCar(nearDeliveryCar)
	...
	if nearDeliveryCar and CanLeaveItem and portacaminhao == true then
		if stock <= limitStock then
			drawTxt("PRESSIONE ~b~E~s~ PARA GUARDAR A ENCOMENDA NO CAMINHÃO", 4,0.5,0.93,0.50,255,255,255,180)
			if IsControlJustPressed(1, 38) then
			...
			drawTxt("O caminhão esta cheio efetue mais entregas", 4,0.5,0.93,0.50,255,255,255,180)
		end
	end
end
...
function getBoxOnCar(nearDeliveryCar2)
	if nearDeliveryCar2 and not CanLeaveItem and GetDistanceBetweenCoords(destination[l].x,destination[l].y,destination[l].z, GetEntityCoords(GetPlayerPed(-1))) < 20.0 and portacaminhao == true then
		if stock >= 1 then
			drawTxt("PRESSIONE ~b~E~s~ PARA PEGAR A ENCOMENDA NO CAMINHÃO", 4,0.5,0.93,0.50,255,255,255,180)
			if (IsControlJustPressed(1, 38)) then
				Citizen.Wait(100)	
				CanLeaveItem = true
				TirarCaixa()
			...
			drawTxt("O caminhão esta vazio busque mais encomendas.", 4,0.5,0.93,0.50,255,255,255,180)
		end
	end
end
...
function openMenuBox(nearBoxFarm)
	if nearBoxFarm and onjob then
		if stock <= limitStock then
			...
			drawTxt("O CAMINHÃO ESTÁ CHEIO, EFETUE ALGUMAS ENTREGAS", 4,0.5,0.93,0.50,255,255,255,180)
		end
	end
end
...
function deliverysucces()
	...
	stock = stock - 1
	if stock == 0 then
		TriggerEvent("Notify","aviso","Não possui mais caixas de frango.")		
	else
		RemoveBlip(blips)
		startjob()
	end
end
function spawnStock4()
	local mhash = "mule4"
	if not nveh then
	 while not HasModelLoaded(mhash) do
	  RequestModel(mhash)
		...
		nveh = CreateVehicle(mhash,-74.62,6272.45,31.38+0.5,40.58,true,false)
...
function CaixaStock(veh,model)
	if stock <= limitStock then
		RequestModel(GetHashKey("prop_paper_box_05"))
        while not HasModelLoaded(GetHashKey("prop_paper_box_05")) do
            Citizen.Wait(10)
        end
	    local coords = GetOffsetFromEntityInWorldCoords(veh,0.0,0.0,-5.0)
		if stock == 0 and model == 1945374990 then
			propcaixa = nil
			propcaixa = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa,veh,0.0,-0.45,-3.2,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa,true)
		elseif stock == 1 and model == 1945374990 then
			propcaixa2 = nil
			propcaixa2 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa2,veh,0.0,0.0,-3.2,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa2,true)
		elseif stock == 2 and model == 1945374990 then
			propcaixa3 = nil
			propcaixa3 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa3,veh,0.0,0.45,-3.2,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa3,true)
		elseif stock == 3 and model == 1945374990 then
			propcaixa4 = nil
			propcaixa4 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa4,veh,0.0,-0.45,-2.8,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa4,true)
		elseif stock == 4 and model == 1945374990 then
			propcaixa5 = nil
			propcaixa5 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa5,veh,0.0,0.0,-2.8,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa5,true)
		elseif stock == 5 and model == 1945374990 then
			propcaixa6 = nil
			propcaixa6 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa6,veh,0.0,0.45,-2.8,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa6,true)
		elseif stock == 6 and model == 1945374990 then
			propcaixa7 = nil
			propcaixa7 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa7,veh,0.0,-0.45,-2.4,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa7,true)
		elseif stock == 7 and model == 1945374990 then
			propcaixa8 = nil
			propcaixa8 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa8,veh,0.0,0.0,-2.4,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa8,true)
		elseif stock == 8 and model == 1945374990 then
			propcaixa9 = nil
			propcaixa9 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa9,veh,0.0,0.45,-2.4,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa9,true)
		elseif stock == 9 and model == 1945374990 then
			propcaixa10 = nil
			propcaixa10 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa10,veh,0.0,-0.45,-2.0,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa10,true)
		elseif stock == 10 and model == 1945374990 then
			propcaixa11 = nil
			propcaixa11 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa11,veh,0.0,0.0,-2.0,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa11,true)
		elseif stock == 11 and model == 1945374990 then
			propcaixa12 = nil
			propcaixa12 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa12,veh,0.0,0.45,-2.0,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa12,true)
		elseif stock == 12 and model == 1945374990 then
			propcaixa13 = nil
			propcaixa13 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa13,veh,0.0,-0.45,-1.6,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa13,true)
		elseif stock == 13 and model == 1945374990 then
			propcaixa14 = nil
			propcaixa14 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa14,veh,0.0,0.0,-1.6,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa14,true)
		elseif stock == 14 and model == 1945374990 then
			propcaixa15 = nil
			propcaixa15 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa15,veh,0.0,0.45,-1.6,0.03,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa15,true)
		elseif stock == 15 and model == 1945374990 then
			propcaixa16 = nil
			propcaixa16 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa16,veh,0.0,-0.45,-1.6,0.34,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa16,true)
		elseif stock == 16 and model == 1945374990 then
			propcaixa17 = nil
			propcaixa17 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa17,veh,0.0,0.0,-1.6,0.34,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa17,true)
		elseif stock == 17 and model == 1945374990 then
			propcaixa18 = nil
			propcaixa18 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa18,veh,0.0,0.45,-1.6,0.34,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa18,true)
		elseif stock == 18 and model == 1945374990 then
			propcaixa19 = nil
			propcaixa19 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa19,veh,0.0,-0.45,-2.0,0.34,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa19,true)
		elseif stock == 19 and model == 1945374990 then
			propcaixa20 = nil
			propcaixa20 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa20,veh,0.0,0.0,-2.0,0.34,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa20,true)
		elseif stock == 20 and model == 1945374990 then
			propcaixa21 = nil
			propcaixa21 = CreateObject(GetHashKey("prop_paper_box_05"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa21,veh,0.0,0.45,-2.0,0.34,0.0,0.0,0.0,false,false,true,false,2,true)
